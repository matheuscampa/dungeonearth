...
```
Em combate:      nível, por turno (HP e PM, cada)
Fora de combate: instantânea — HP e PM enchem por completo
```

**Em combate**, no início de cada turno seu, recupere HP e PM iguais ao seu **nível** (nível 1 = +1/+1; nível 5 = +5/+5) — marginal diante do dano, é só o fôlego natural do crawler. **Fora de combate a regeneração é instantânea:** assim que o combate termina (ou a qualquer momento fora dele), suas barras de HP e PM enchem por completo na hora, sem precisar contar turnos. Por isso a cura do dia a dia não depende de poções nem da Sala Segura — você se regenera sozinho, na mesma hora. Enquanto **Caído** (0 HP) você **não** regenera, dentro ou fora de combate: faça Testes de Morte (§8) — alguém precisa te estabilizar.

> **Exceção — Exaustão.** A partir do nível 2 de Exaustão (§14.1), a regeneração fora de combate deixa de ser instantânea: volta a ser gradual, à metade do nível por turno. É o corpo do crawler cobrando a conta do descanso pulado.
...
### PA é um placar — nunca se gasta

O **PA é a sua Audiência acumulada na carreira**: a soma de todo PA que você já ganhou na campanha. **É um placar que só sobe** — você **nunca gasta PA**. Quanto maior o número, maior o seu prestígio no show, e é ele que define o seu **nível de patrocinador** (e potencializa skills que escalam com Audiência, como Cura da Torcida).

### Patrocinadores (a partir do Andar 3)

Quando o show ganha tração — **do Andar 3 em diante** — cada crawler atrai patrocinadores conforme o **PA** acumulado. A cada andar, você recebe **1 Caixa de Patrocínio grátis** (uma por crawler, por andar), do tier abaixo:

| PA acumulado | Patrocinador | Caixa de Patrocínio (1×/andar) |
|---|---|---|
| 30+ | **Local** | Prata |
| 60+ | **Regional** | Ouro |
...
*(Abaixo de 30 de PA, ou antes do Andar 3, ainda não há patrocínio fixo — mas a skill Patrocínio Relâmpago saca uma Caixa de Patrocínio extra do seu tier atual.)*
...
Ou seja: o PA não é enfeite. Um crawler que se joga para a plateia sai dos andares profundos com itens visivelmente melhores que o grupo "careta".
...
**Descanso Diário (obrigatório).** A cada "dia" da masmorra (8 horas de ficção), todo crawler **precisa** dormir um turno. O descanso **zera cooldowns de recarga "descanso"** e **remove 1 nível de Exaustão** e condições de longa duração (HP/PM já enchem sozinhos fora de combate — §3.4.1 — então o Descanso não precisa fazer isso). Sem a **Cama**, o descanso consome as 8 horas in-fiction — o que **avança o relógio do andar** (não durma em lugar errado).
...
**Exaustão (condição graduada).** Pular o Descanso Diário, ou esforços extremos (farm prolongado, certas falhas críticas, efeitos de inimigo), acumulam **níveis de Exaustão**. Cada nível é cumulativo:

| Nível | Efeito (cumulativo) |
|---|---|
| **1** | −1 em todas as rolagens (Precisão, Defesa, resistências, perícias). |
| **2** | A regeneração fora de combate deixa de ser instantânea (volta a ser gradual, à metade do nível por turno); HP/PM máximos −10%. |
| **3** | Velocidade de deslocamento à metade; não pode correr nem usar reações. |
| **4** | A cada turno de combate, role d20: em 1–5, fica Atordoado naquele turno. |
| **5** | **Colapso:** o crawler cai inconsciente até receber um Descanso completo ou cura mágica de Exaustão. |

**Como ganha:** +1 nível a cada "dia" (8h) sem cumprir o Descanso Diário; +1 ao farmar 3 turnos seguidos sem descansar (ver §14.4); efeitos específicos podem impor mais. **Como remove:** um **Descanso Diário completo remove 1 nível** (Cama remove na hora, sem gastar as 8h); poções/skills específicas podem remover níveis extras. A Exaustão **persiste entre combates** até ser descansada — é o relógio biológico do crawler.
...
### 14.4 Farm (caçar e se preparar)

Um andar não precisa ser atravessado em linha reta. O grupo pode **farmar**: caçar os bichos errantes do andar para **subir de nível**, **juntar loot** e se preparar antes do chefe ou do próximo andar. Custa **tempo** — e tempo, na masmorra, é perigoso.

**Como funciona.** Fora de uma Zona Segura, o grupo gasta um **turno de 8 horas** (a mesma unidade do Descanso Diário, §14.1) farmando. A cada turno de farm:

- **XP:** o Mestre saca **1d4+1 inimigos errantes** do bestiário do andar (hordas de Lacaios/Padrão). Resolva o combate normalmente — ou, para agilizar, conceda o XP somado e siga. (XP normal: Tier × andar; **golpe final = dobro**.)
- **Loot:** cada crawler ganha **1 Caixa de Loot do tier do andar** (Andar 1 = Bronze; sobe conforme se desce) + um pouco de ouro. Skills de forrageamento/saque rendem extra.
- **Tempo:** o turno gasta 8 horas — **avança o relógio de colapso** e conta para o **Descanso Diário obrigatório**. Farmar demais faz o andar fechar antes da hora.

**O risco — Gatilho de Batalha Épica.** A masmorra (e o Sistema, sedento por audiência) não gosta de quem se demora. Ao **fim de cada turno de farm**, role o gatilho:

```
Gatilho de Batalha Épica:  ~33%   (d100 ≤ 33, ou 1-2 num d6)
```

Se disparar, começa uma **Batalha Épica** — um encontro set-piece **extremamente perigoso**, muito acima do conteúdo normal do andar. As recompensas também são enormes (XP de chefe, caixas de tier alto, itens que definem build), mas o risco de morte é real.

> **Escalada (recomendado):** a cada turno de farm **consecutivo** no mesmo andar, **+1 na faixa do gatilho** — turno 1 = 1-2 (33%) · turno 2 = 1-3 (50%) · turno 3 = 1-4 (67%)… A masmorra perde a paciência: farmar é um risco **crescente**, não um moedor infinito.

> **Cansaço do farm:** farmar **3 turnos seguidos sem cumprir o Descanso Diário** impõe **+1 nível de Exaustão** (ver §14.1). O corpo cobra o moedor.

As Batalhas Épicas de cada andar ficam no módulo do andar (as do Andar 1 estão em `DCC-Modulo-Andar1.md`).
...
| **Regeneração** | fora de combate: instantânea · em combate: nível/turno (HP e PM) |
...
**PA** é placar de Audiência — **só sobe, nunca se gasta** — e define o patrocinador.
**Patrocínio (Andar 3+):** 1 Caixa grátis/andar por PA — 30:Prata · 60:Ouro · 100:Platina · 160:Lendária · 250:Divina. **Premium:** sem Sucata · +1 raridade · role 2× e escolha.

### Sala Segura & Descanso
Sem combate: abrir Caixas · organizar/reequipar · falar com o Guia · loja. HP/PM já regeneram sozinhos e na hora fora de combate (§3.4.1) — a Sala Segura não cura, só dá espaço pro resto. **Descanso Diário (8h, obrigatório):** zera cooldowns de "descanso", remove **1 nível de Exaustão** e condições longas. **Cama** (upgrade) = descansa na hora.
**Exaustão (acumula sem dormir / farm longo):** N1 −1 em tudo · N2 regen fora de combate volta a gradual (½ nível/turno) e HP/PM máx −10% · N3 desloc ½, sem reações · N4 d20≤5 = Atordoado · N5 colapsa. Descanso remove 1 nível.

### Farm (caçar · turno de 8h)
Fora de Zona Segura: saca **1d4+1 errantes** (XP) + **1 Caixa do tier do andar por crawler** + ouro. Gasta 8h (relógio de colapso + Descanso Diário). **Fim do turno → gatilho de Batalha Épica ~33% (d6 1-2)**, +1 na faixa por turno consecutivo. Batalha Épica = set-piece **extremamente perigoso**, recompensa enorme (ver módulo do andar).
...
*Sistema Central v1.5 — documento vivo. Novidades v1.5: PA virou placar de Audiência único (só sobe, nunca se gasta) — define o patrocinador; Exaustão formalizada como condição graduada (0-5) em §14.1; XP de conquistas reescalado à nova curva. v1.4: mecânica de Farm (§14.4) — caçar por turnos de 8h por XP e loot, com gatilho de Batalha Épica (~33%). v1.3: regras de XP (individual, golpe final = dobro, curva sem teto, ~6 níveis/andar) e Audiência/Patrocínio (patrocinadores a partir do Andar 3, Caixas de Patrocínio premium). v1.2: §6A Combate avançado e novas condições (Derrubado, Medo, Imobilizado, Provocado, Oculto). Materiais existentes: bestiário (com XP por carta), falha crítica, tabela de loot, lista de itens (175, com tipo de dano), lista de skills (223), cartas de condição, loadout, conquistas, companheiros, módulo do Andar 1, ficha do jogador e dashboard. Pendências organizadas em `DCC-Backlog.md`.*
