...
*Documento vivo. Atualizado em 2026-07-14. Marque `[x]` quando concluir.*
...
- [x] **Correção: travava na tela do d100 ao abrir Caixa.** Item entrava no inventário mas o modal não avançava — causa raiz: um erro em qualquer parte do pipeline de `render()` (chamado logo antes de mostrar o resultado) interrompia a função inteira antes de chegar em `renderBoxResult()`, deixando a tela de "digite o d100" travada. Corrigido isolando cada etapa (`resolveBox`, `persistState`, `render`, `renderBoxResult`) em try/catch própria — agora o resultado (ou um aviso do que deu errado) sempre aparece, e erros ficam logados no Console (`[DCC] ...`) para diagnóstico futuro. Testado com 300 combinações de tier/rolagem/premium sem travar. Feito.
...
- [x] **Correção: falso "falha ao enviar o item".** Ao enviar item pra outro jogador, o Firebase às vezes rejeitava a *promise* de confirmação mesmo com a escrita concluída (fila de retry/offline), disparando o alerta de erro apesar do item ter chegado. Agora, se a confirmação falha, o código **verifica se o item foi mesmo entregue** antes de alarmar — só mostra erro de verdade se não foi. Feito no `DCC-Dashboard-Equipamento.html`.
- [x] **Correção: banner vermelho de "ficha de outro jogador" removido.** O aviso (FOREIGN_SHEET/"dono do dispositivo") ficou obsoleto com o login de verdade por conta — cada jogador tem sua própria sessão autenticada, não depende mais de qual link/dispositivo abriu primeiro. Lógica e banner removidos do `DCC-Dashboard-Equipamento.html`.
- [x] **Correção: conquistas sumindo.** Causa raiz — `pushCloud()` sobrescrevia o nó inteiro do jogador na nuvem a cada salvamento (HP, ouro, etc.), incluindo o objeto de conquistas em memória; se uma aba/dispositivo desatualizado (sem uma conquista marcada em outra sessão) salvasse qualquer coisa, apagava essa conquista da nuvem. Corrigido: `pushCloud()` agora lê as conquistas atuais da nuvem e faz merge (união) com as locais antes de sobrescrever; desmarcar uma conquista agora também remove direto e na hora daquele item específico na nuvem, em vez de depender só do próximo save geral. Feito no `DCC-Dashboard-Equipamento.html`.
